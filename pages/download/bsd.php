...
foreach ($artifacts as $architecture => $files) {
	...
	foreach ($all_keys as $key) {
		if (array_key_exists($key, $fmt_mapping)) {
			...
			echo "<a class=\"download-text-button bsd-arch formats-links {$fmt}-dwnld\" href=\"{$url}\" alt=\"{$alt}\">{$arch}</a>\n";
		} else {
			echo "<a class=\"download-text-button bsd-arch formats-links-inactive {$fmt}-dwnld\" href=\"#\" alt=\"\">{$arch}</a>\n";
		}
	}
}
?>
