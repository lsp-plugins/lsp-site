...
function dao_get_paddle_product($db, $test, $name) {
	$stmt = null;
	try {
...
function dao_create_paddle_product($db, $test, $name, $product_id) {
	$stmt = null;
	
	try {
		$stmt = mysqli_prepare($db, "INSERT INTO paddle_products(name, product_id, test) VALUES (?, ?, ?)");
...
function dao_get_paddle_price($db, $test, $product_id, $amount) {
	$stmt = null;
	try {
		$stmt = mysqli_prepare($db,
			"SELECT " .
...
function dao_create_paddle_price($db, $test, $product_id, $price_id, $amount) {
	$stmt = null;
	
	try {
		$stmt = mysqli_prepare($db, "INSERT INTO paddle_prices(product_id, price_id, amount, test) VALUES (?, ?, ?, ?)");
